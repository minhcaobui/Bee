...
public class DashboardService {
    ...
    public ResponseEntity<?> layChiSo(String period, String from, String to) {
    ...
    private Map<String, Object> emptyStats() {
        ...
        m.put("pend", 0L); m.put("avg", 0.0); m.put("avgChg", 0.0); m.put("avgUp", true);
        m.put("cust", 0L); m.put("custChg", 0.0); m.put("custUp", true);
        m.put("sold", 0L); m.put("rate", 0L); m.put("cancelled", 0L);
        m.put("pureCompleted", 0L); m.put("exchanged", 0L); m.put("returned", 0L);
        return m;
    }

    public ResponseEntity<?> layBieuDo(String period, String from, String to) {
        LocalDateTime[] cur = getRange(period, from, to);
        LocalDateTime[] prev = getPrevRange(period, cur);
    ...
    public ResponseEntity<?> layPhanTichAI() {
        ...
        insights.add(Map.of("type", "info", "msg", "Dữ liệu Heatmap cho thấy khách hàng thường chốt đơn mạnh vào 10h-12h trưa và 19h-21h tối. Đây là khung giờ vàng để chạy Flash Sale."));
        return ResponseEntity.ok(insights);
    }

    public ResponseEntity<?> laySanPhamBanChay(String period, String from, String to) {
        LocalDateTime[] cur = getRange(period, from, to);
    ...
    public ResponseEntity<?> layPhuongThucThanhToan(String period, String from, String to) {
        LocalDateTime[] cur = getRange(period, from, to);
        ...
        return ResponseEntity.ok(Map.of("keys", new ArrayList<>(aggregated.keySet()), "values", new ArrayList<>(aggregated.values())));
    }
    ...
    public ResponseEntity<?> layBieuDoNhiet(String period, String from, String to) {
        LocalDateTime[] cur = getRange(period, from, to);
    ...
    public ResponseEntity<?> xuLyChatbot(Map<String, String> payload) {
