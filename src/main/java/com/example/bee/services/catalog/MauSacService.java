...
public class MauSacService {
    ...
    private String taoMaTuDong() {
        ...
        return "MS" + timeStr.substring(timeStr.length() - 5);
    }

    public Page<MauSac> layDanhSach(String q, Boolean trangThai, int page, int size) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("id").descending());
        return mauSacRepository.search(q, trangThai, pageable);
    }

    public List<MauSac> layTatCaHoatDong() {
        return mauSacRepository.findByTrangThaiTrue();
    }

    public MauSac layChiTiet(Integer id) {
        return mauSacRepository.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Không tìm thấy dữ liệu!"));
    }

    @Transactional
    public MauSac taoMoi(MauSac body) {
    ...
    @Transactional
    public MauSac capNhat(Integer id, MauSac body) {
